...
namespace HTMLHelperPagination.Common
{
        ...
        (this IHtmlHelper htmlHelper, PageInfo pageInfo, Func<int, string> PageUrl)
        {
            ...
            if (pageInfo.CurrentPage > 1)
            {
                pagingTags.Append(GetTagString
                                 ("Prev ", PageUrl(pageInfo.CurrentPage - 1), pageInfo.CurrentPage.ToString()));
            }
            ...
            {
                pagingTags.Append(GetTagString
                                 (" Next", PageUrl(pageInfo.CurrentPage + 1), pageInfo.CurrentPage.ToString()));
            }
            //paging tags
            return new HtmlString(pagingTags.ToString());
        }

        private static string GetTagString(string innerHtml, string hrefValue, string currPage)
        {
